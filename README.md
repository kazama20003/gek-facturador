...
- Motivo inválido para el catálogo → 422.
- **Flujo persistido**: `POST /credit-notes` | `POST /debit-notes` (guarda; duplicado → 409) → `GET /notes/:id` → `POST /notes/:id/sunat/send` (envía y guarda CDR/estado). Migración `0002_notes`.
